...
I tried recalibrating thresholds per batch, using each image's own feature percentiles instead of the fixed synthetic values. That made predictions non-degenerate, but the resulting accuracy, 23.9\%, was worse than simply guessing the majority class (73.9\% at the time), so I reverted it rather than count it as progress.
